...
const FlipCard = ({images}) => {
    ...
    function nextImageIndex() {
        if(isFlipped) {
            setFrontImage(images[imgIndex + 1 >= images.length ? (imgIndex + 1 == images.length ? 0 : 1) : imgIndex + 1]);
        } else {
            setBackImage(images[imgIndex + 1 >= images.length ? (imgIndex + 1 == images.length ? 0 : 1) : imgIndex + 1]);
        }
    ...
    return (
        <div className="
            w-full aspect-video
            flex flex-col justify-center items-center" onClick={flip}>
